 ...
 */
public class DOMAnalyzerEnhanced {

    private static Logger log = LoggerFactory.getLogger(DOMAnalyzerEnhanced.class);
     ...
     */
    public DOMAnalyzerEnhanced(org.w3c.dom.Document doc) {
        this.doc = doc;
     ...
     *
     * @param doc the document to be analyzed
     * @param baseUrl the base URL for loading the style sheets. This URL may be
     * redefined by the <code>&lt;base&gt;</code> tag used in the document
     * header.
     */
    public DOMAnalyzerEnhanced(org.w3c.dom.Document doc, URL baseUrl) {
     ...
     *
     * @param doc the document to be analyzed
     * @param baseUrl the base URL for loading the style sheets. If
     * <code>detectBase</code>, this URL may be redefined by the
     * <code>&lt;base&gt;</code> tag used in the document header.
     * @param detectBase sets whether to try to accept the
     * <code>&lt;base&gt;</code> tags in the document header.
     */
    public DOMAnalyzerEnhanced(org.w3c.dom.Document doc, URL baseUrl, boolean detectBase) {
        this.doc = doc;
        this.encoding = null;
        this.media = new MediaSpec(DEFAULT_MEDIA);
        styles = new Vector<StyleSheet>();
    ...
    /**
     * Obtains the default character encoding used for parsing the style sheets.
     * This encoding is used when there is no other encoding specified inside
     * the style sheet using the <code>@charset</code> rule. When there is no
     * default encoding specified, this method returns <code>null</code> and the
     * system default encoding is used for style sheets.
     *
     * @return The encoding name or <code>null</code> when there is no default
     * encoding specified.
     */
    ...
    /**
     * Sets the default character encoding used for parsing the referenced style
     * sheets. This encoding is used when there is no other encoding specified
     * inside the style sheet using the <code>@charset</code> rule.
     *
     * @param encoding The encoding string or <code>null</code> for using the
    ...
    /**
     * Set the type of the medium used for computing the style. Default is
     * "screen".
     *
    ...
    /**
     * Set the specification of the medium used for computing the style. The
     * default is "screen" with some standard features that correspond to a
     * normal desktop station.
     *
     * @param media the medium to set
     */
    ...
    /**
     * Finds the explicitly specified base URL in the document according to the
     * HTML specification
     *
     * @return the base URL string, when present in the document or null when
     * not present
    ...
    /**
     * Finds the explicitly specified character encoding using the
     * <code>&lt;meta&gt;</code> tag according to the HTML specifiaction.
     *
     * @return the encoding name when present in the document or
    ...
    /**
     * Formats the tag tree to an output stream. This is used mainly for
     * debugging purposes.
     *
    ...
    /**
     * Encodes the efficient style of all the elements to their
     * <code>style</code> attributes. Inheritance is not applied. This is
     * currently not necessary for the rendering. It is practical mainly
     * together with the <code>printTagTree</code> method for debugging the
     * resulting style.
     */
    ...
    /**
     * Encodes the efficient style of all the elements to their
     * <code>style</code> attributes while applying the inheritance. This is
     * currently not necessary for the rendering. It is practical mainly
     * together with the <code>printTagTree</code> method for debugging the
     * resulting style.
     */
    ...
    /**
     * Converts the HTML presentation attributes in the document body to inline
     * styles.
     */
    ...
    /**
     * Removes all the external style sheet links and puts the whole style sheet
     * locally to the document head.
     */
    ...
    /**
     * Returns a vector of CSSStyleSheet objects referenced from the document
     * for the specified media type with default values of the remaining media
     * features. The internal style sheets are read from the document directly,
     * the external ones are downloaded and parsed automatically.
     *
    ...
    /**
     * Returns a vector of CSSStyleSheet objects referenced from the document
     * for the specified media type and features. The internal style sheets are
     * read from the document directly, the external ones are downloaded and
    ...
    /**
     * Returns a vector of CSSStyleSheet objects referenced from the document
     * for the media type set by <code>setMedia()</code> (or "screen" by
     * default). The internal style sheets are read from the document directly,
     * the external ones are downloaded and parsed automatically.
     */
    ...
    /**
     * Loads a stylesheet from an URL as na author style sheet. Imports all the
     * imported style sheets before storing it.
     *
    ...
    /**
     * Loads a stylesheet from an URL and specifies the origin. Imports all the
     * imported style sheets before storing it.
     *
     * @param base the document base url
     * @param href the href specification
     * @param encoding the default character encoding for the style sheet (use
     * <code>null</code> for default system encoding)
    ...
    /**
     * Parses and adds an author style sheet represented as a string to the end
     * of the used stylesheet lists. It imports all the imported style sheets
     * before storing it.
     *
     * @param base the document base URL
     * @param cssdata the style string
     * @deprecated This method does not specify the style sheet origin; use
     * {@link #addStyleSheet(URL, String, Origin)} instead.
     */
     ...
     * Parses and adds an author style sheet represented as a string to the end
     * of the used stylesheet lists. It imports all the imported style sheets
     * before storing it. The origin of the style sheet is set to the author,
     * agent or user. The origin influences the rule priority according to the
    ...
    public void addStyleSheet(URL base, String cssdata, Origin origin) {
        try {
            StyleSheet newsheet = CSSFactoryEnhanced.parse(cssdata, base);
            newsheet.setOrigin(translateOrigin(origin));
            styles.add(newsheet);
        } catch (IOException e) {
            log.error("I/O Error: " + e.getMessage());
    ...
    /**
     * Forces recomputing the element styles. This method should be called when
     * a new style sheet has been added or a DOM has changed after some styles
     * have been read from the analyzer.
     */
    ...
    /**
     * Gets all the style declarations for a particular element and computes the
     * resulting element style.
     *
    ...
    /**
     * Checks whether the inhetited style has been computed and computes it when
     * necessary.
     */
    ...
    /**
     * Gets all the style declarations for a particular element and computes the
     * resulting element style including the inheritance from the parent.
     *
     * @param el the element for which the style should be computed
    ...
    /**
     * Gets all the style declarations for a particular element and a particular
     * pseudo-element and computes the resulting pseudo-element style including
     * the inheritance from the parent.
     *
     * @param el the element for which the style should be computed
    ...
    /**
     * Checks whether the element has some style defined for a specified
     * pseudo-element.
     *
     * @param el The element to be checked
     * @param pseudo The pseudo element specification
     * @return <code>true</code> if there is some style defined for the given
     * pseudo-element
     */
    ...
    /**
     * Translates the origin from the CSSBox API to jStyleParser API (in order
     * not to expose the jStyleParser API in CSSBox)
     */
